...
import { visionChat } from "./ai-provider.js";
...
export function getAllMcpTools(): AgentTool<any>[] {
	return [
		...
		// Four GOATs Integration (Mario, Simon, Daniel, Dan)
		createLlmQueryTool(), // Simon Willison's LLM CLI
		createFabricPatternTool(), // Daniel Miessler's Fabric patterns
		createSqliteQueryTool(), // Simon Willison's sqlite-utils
		createFabricListTool(), // List available Fabric patterns
		createComputeAdvantageTool(), // IndyDevDan's Compute Advantage calculator
		createTacTacticsTool(), // IndyDevDan's 8 TAC Tactics
		createLeveragePointsTool(), // IndyDevDan's 12 Leverage Points

		// just-bash Integration (Vercel - "Bash is all you need")
		createJustBashTool(), // Secure sandboxed bash execution
		createVercelInsightsTool(), // Vercel's 80% tool reduction insights
	];
}
...
const infiniteLoopSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	spec: Type.String({ description: "Specification content or path to spec file" }),
	output_dir: Type.String({ description: "Output directory for generated iterations" }),
	count: Type.Union([Type.Number(), Type.Literal("infinite")], {
		description: "Number of iterations (1-N) or 'infinite' for continuous generation",
	}),
	wave_size: Type.Optional(Type.Number({ description: "Agents per wave (default: 3)" })),
});

/**
 * Infinite Agentic Loop - Parallel sub-agent orchestration for iterative generation
 */
export function createInfiniteLoopTool(): AgentTool<typeof infiniteLoopSchema> {
	return {
		name: "infinite_loop",
		label: "infinite_loop",
		description:
			"Orchestrate parallel sub-agents for iterative content generation. Spawns waves of agents that each generate unique iterations based on a specification. Use for bulk generation, variations, or continuous improvement loops.",
		parameters: infiniteLoopSchema,
		execute: async (_toolCallId, { spec, output_dir, count, wave_size = 3, label }) => {
			logMcpTool("infinite_loop", label);
			...
			try {
				...
				// Read spec if it's a file path
				let specContent = spec;
				if (spec.endsWith(".md") || spec.startsWith("/") || spec.startsWith("~")) {
					try {
						const specPath = spec.startsWith("~") ? spec.replace("~", process.env.HOME || "") : spec;
						specContent = await fs.readFile(specPath, "utf-8");
					} catch {
						// Use spec as content if file read fails
					}
				}
				...
				for (const file of existingFiles) {
					...
					if (match) {
						...
						if (num >= startIteration) startIteration = num + 1;
					}
				}

				// Determine total iterations
				const isInfinite = count === "infinite";
				const totalIterations = isInfinite ? 100 : (count as number); // Cap infinite at 100
				const results: string[] = [];
				let currentIteration = startIteration;
				let waveNum = 0;

				// Wave-based generation
				while (currentIteration < startIteration + totalIterations) {
					waveNum++;
					const waveIterations = Math.min(wave_size, startIteration + totalIterations - currentIteration);

					if (waveIterations <= 0) break;

					// Launch wave of agents
					const wavePromises = [];
					for (let i = 0; i < waveIterations; i++) {
						const iterNum = currentIteration + i;
						const task = `Generate iteration ${iterNum} based on this spec. Output to ${output_dir}/iteration_${iterNum}.md. Spec: ${specContent.substring(0, 1500)}`;

						wavePromises.push(
							(async () => {
								try {
									const cmd = `timeout 45 opencode run "${task.replace(/"/g, '\\"').replace(/\n/g, " ")}" 2>&1 | tail -20`;
									const { stdout } = await execAsync(cmd, { maxBuffer: 512 * 1024 });
									return { iter: iterNum, success: true, output: stdout.substring(0, 300) };
								} catch (e) {
									return { iter: iterNum, success: false, output: getErrorMessage(e) };
								}
							})(),
						);
					}
					...
					results.push(`Wave ${waveNum}: ${successCount}/${waveIterations} iterations completed`);

					currentIteration += waveIterations;

					// For infinite mode, check if we should continue (simplified - just do configured waves)
					if (isInfinite && waveNum >= 5) {
						results.push("Infinite mode: Completed 5 waves, pausing for context management");
						break;
					}
				}
				...
				const summary = [
					...
					`- Output: ${output_dir}`,
					`- Iterations: ${startIteration} to ${currentIteration - 1}`,
					`- Waves: ${waveNum}`,
					``,
					`**Results:**`,
					...results,
				...
				return {
					content: [{ type: "text", text: `Infinite loop failed: ${getErrorMessage(error)}` }],
					details: undefined,
				};
			}
		},
...
// =============================================================================
// Four GOATs Tools (Simon Willison, Daniel Miessler)
// =============================================================================

const llmQuerySchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
...
/**
 * LLM Query - Simon Willison's LLM CLI
 */
export function createLlmQueryTool(): AgentTool<typeof llmQuerySchema> {
	return {
		name: "llm_query",
		label: "llm_query",
		description: "Query any LLM using Simon Willison's llm CLI. Supports multiple providers via plugins.",
		parameters: llmQuerySchema,
		execute: async (_toolCallId, { prompt, model, system, label }) => {
			logMcpTool("llm_query", label);

			try {
				const { execSync } = await import("child_process");
				const modelArg = model ? `-m ${model}` : "";
				const systemArg = system ? `-s "${system.replace(/"/g, '\\"')}"` : "";
				const cmd = `llm ${modelArg} ${systemArg} "${prompt.replace(/"/g, '\\"')}" 2>&1 | head -100`;

				const result = execSync(cmd, { encoding: "utf-8", timeout: 60000, maxBuffer: 1024 * 1024 }).trim();
				...
				return {
					content: [{ type: "text", text: `LLM query failed: ${getErrorMessage(err)}` }],
					details: undefined,
				};
			}
		},
...
const fabricPatternSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	pattern: Type.String({ description: "Fabric pattern name (e.g., summarize, extract_wisdom, analyze_claims)" }),
	input: Type.String({ description: "Input text to process" }),
	model: Type.Optional(Type.String({ description: "Model to use" })),
});

/**
 * Fabric Pattern - Daniel Miessler's AI patterns
 */
export function createFabricPatternTool(): AgentTool<typeof fabricPatternSchema> {
	return {
		name: "fabric_pattern",
		label: "fabric_pattern",
		description:
			"Apply Daniel Miessler's Fabric AI patterns. 242+ patterns for summarize, extract_wisdom, analyze_claims, write_essay, code_review, etc.",
		parameters: fabricPatternSchema,
		execute: async (_toolCallId, { pattern, input, model, label }) => {
			logMcpTool("fabric_pattern", label);

			try {
				const { execSync } = await import("child_process");
				const modelArg = model ? `-m ${model}` : "";
				// Escape input for shell and pipe to fabric
				const escapedInput = input.replace(/'/g, "'\\''");
				const cmd = `echo '${escapedInput}' | fabric -p ${pattern} ${modelArg} 2>&1 | head -200`;

				const result = execSync(cmd, { encoding: "utf-8", timeout: 120000, maxBuffer: 2 * 1024 * 1024 }).trim();

				return {
					content: [{ type: "text", text: result.substring(0, 6000) }],
					details: undefined,
				};
			} catch (err) {
				return {
					content: [{ type: "text", text: `Fabric pattern failed: ${getErrorMessage(err)}` }],
					details: undefined,
				};
			}
		},
...
/**
 * SQLite Query - Simon Willison's sqlite-utils
 */
export function createSqliteQueryTool(): AgentTool<typeof sqliteQuerySchema> {
	return {
		name: "sqlite_query",
		label: "sqlite_query",
		description: "Query SQLite databases using Simon Willison's sqlite-utils. Returns JSON output.",
		parameters: sqliteQuerySchema,
		execute: async (_toolCallId, { database, query, label }) => {
			logMcpTool("sqlite_query", label);

			try {
				const { execSync } = await import("child_process");
				const escapedQuery = query.replace(/"/g, '\\"');
				const cmd = `sqlite-utils query "${database}" "${escapedQuery}" --json 2>&1 | head -100`;

				const result = execSync(cmd, { encoding: "utf-8", timeout: 30000, maxBuffer: 1024 * 1024 }).trim();
				...
				return {
					content: [{ type: "text", text: `SQLite query failed: ${getErrorMessage(err)}` }],
					details: undefined,
				};
			}
		},
...
const fabricListSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	filter: Type.Optional(Type.String({ description: "Filter patterns by keyword" })),
});

/**
 * Fabric List - List available Fabric patterns
 */
export function createFabricListTool(): AgentTool<typeof fabricListSchema> {
	return {
		name: "fabric_list",
		label: "fabric_list",
		description: "List available Fabric patterns. Use filter to search for specific patterns.",
		parameters: fabricListSchema,
		execute: async (_toolCallId, { filter, label }) => {
			logMcpTool("fabric_list", label);

			try {
				const { execSync } = await import("child_process");
				const grepArg = filter ? `| grep -i "${filter}"` : "";
				const cmd = `fabric -l 2>&1 ${grepArg} | head -50`;

				const result = execSync(cmd, { encoding: "utf-8", timeout: 10000 }).trim();

				return {
					content: [{ type: "text", text: result || "No patterns found" }],
					details: undefined,
				};
			} catch (err) {
				return {
					content: [{ type: "text", text: `Fabric list failed: ${getErrorMessage(err)}` }],
					details: undefined,
				};
			}
		},
...
// =============================================================================
// IndyDevDan TAC Tools (8 Tactics, 12 Leverage Points, Compute Advantage)
// =============================================================================

const computeAdvantageSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	compute_scaling: Type.Number({ description: "Compute scaling factor (1-100)" }),
	autonomy: Type.Number({ description: "Agent autonomy level (1-100)" }),
	time_hours: Type.Number({ description: "Time investment in hours" }),
	effort_percent: Type.Number({ description: "Effort percentage (1-100)" }),
	monetary_cost: Type.Number({ description: "Monetary cost in USD" }),
});

/**
 * Compute Advantage Calculator - Dan's equation for agentic ROI
 * Compute Advantage = (Compute Scaling × Autonomy) / (Time + Effort + Cost)
 */
export function createComputeAdvantageTool(): AgentTool<typeof computeAdvantageSchema> {
	return {
		name: "compute_advantage",
		label: "compute_advantage",
		description:
			"Calculate Compute Advantage using IndyDevDan's equation: (Compute × Autonomy) / (Time + Effort + Cost). Higher is better.",
		parameters: computeAdvantageSchema,
		execute: async (_toolCallId, { compute_scaling, autonomy, time_hours, effort_percent, monetary_cost, label }) => {
			logMcpTool("compute_advantage", label);

			// Normalize inputs
			const compute = Math.min(100, Math.max(1, compute_scaling));
			const auto = Math.min(100, Math.max(1, autonomy));
			const time = Math.max(0.1, time_hours);
			const effort = Math.min(100, Math.max(1, effort_percent));
			const cost = Math.max(1, monetary_cost);

			// Calculate advantage
			const numerator = compute * auto;
			const denominator = time + effort / 10 + cost / 100;
			const advantage = numerator / denominator;

			// Interpret score
			let interpretation: string;
			if (advantage > 500) interpretation = "EXCELLENT: High leverage, maximize this approach";
			else if (advantage > 200) interpretation = "GOOD: Worth pursuing";
			else if (advantage > 50) interpretation = "MODERATE: Consider optimization";
			else interpretation = "LOW: Reconsider approach or increase autonomy";

			return {
				content: [
					{
						type: "text",
						text: `**Compute Advantage Analysis**

📊 **Inputs:**
- Compute Scaling: ${compute}/100
- Autonomy Level: ${auto}/100
- Time Investment: ${time}h
- Effort: ${effort}%
- Cost: $${cost}

🧮 **Calculation:**
(${compute} × ${auto}) / (${time} + ${effort / 10} + ${cost / 100})
= ${numerator} / ${denominator.toFixed(2)}

🎯 **Compute Advantage: ${advantage.toFixed(1)}**

${interpretation}

💡 **12 Leverage Points to improve:**
Core Four: Context, Model, Prompt, Tools
Extended: stdout, types, tests, architecture, documentation, feedback, monitoring, learning`,
					},
				],
				details: undefined,
			};
...
const tacTacticsSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	tactic: Type.Optional(
		Type.Number({ description: "Specific tactic number (1-8) or omit for all", minimum: 1, maximum: 8 }),
	),
});

/**
 * TAC Tactics Reference - IndyDevDan's 8 Tactics for Agentic Coding
 */
export function createTacTacticsTool(): AgentTool<typeof tacTacticsSchema> {
	return {
		name: "tac_tactics",
		label: "tac_tactics",
		description:
			"Get IndyDevDan's Tactical Agentic Coding tactics. Use for understanding how to build autonomous agent systems.",
		parameters: tacTacticsSchema,
		execute: async (_toolCallId, { tactic, label }) => {
			logMcpTool("tac_tactics", label);

			const tactics = [
				{
					num: 1,
					name: "Hello Agentic Coding",
					level: "Beginner",
					desc: "Transition from traditional coding to agent-based engineering. Stop coding, start prompting.",
					key: "The prompt is the new fundamental unit of work.",
				},
				{
					num: 2,
					name: "The 12 Leverage Points",
					level: "Beginner",
					desc: "Maximize agent autonomy through the Core Four (Context, Model, Prompt, Tools) plus 8 extended points.",
					key: "Maximize not what YOU can do, but what your AGENTS can do.",
				},
				{
					num: 3,
					name: "Success is Planned",
					level: "Intermediate",
					desc: "Strategic planning to enable agent execution. Define clear outcomes before agent runs.",
					key: "Plan the work, then let agents work the plan.",
				},
				{
					num: 4,
					name: "AFK Agents",
					level: "Intermediate",
					desc: "Fully autonomous agents via the PITER framework. Agents that work while you're away.",
					key: "Away From Keyboard = agents running without supervision.",
				},
				{
					num: 5,
					name: "Close The Loops",
					level: "Intermediate",
					desc: "Self-correcting agent systems with feedback mechanisms. Test → Fix → Verify cycles.",
					key: "Agents should detect and fix their own mistakes.",
				},
				{
					num: 6,
					name: "Let Your Agents Focus",
					level: "Advanced",
					desc: "Specialized Review and Documentation agents. Each agent has one job.",
					key: "Single-purpose agents outperform multi-purpose agents.",
				},
				{
					num: 7,
					name: "ZTE (Zero-Touch Engineering)",
					level: "Advanced",
					desc: '"Your codebase ships itself" paradigm. Full CI/CD automation via agents.',
					key: "Human approval only at deployment gates.",
				},
				{
					num: 8,
					name: "The Agentic Layer",
					level: "Advanced",
					desc: "Meta-tactic unifying the methodology. Agents building agents.",
					key: "Build systems that build systems.",
				},
			];

			if (tactic && tactic >= 1 && tactic <= 8) {
				const t = tactics[tactic - 1];
				return {
					content: [
						{
							type: "text",
							text: `**TAC Tactic ${t.num}: ${t.name}** (${t.level})

${t.desc}

🔑 **Key Insight:** "${t.key}"

📚 Learn more: https://agenticengineer.com/tactical-agentic-coding`,
						},
					],
					details: undefined,
				};
			...
			const allTactics = tactics
				.map((t) => `**${t.num}. ${t.name}** (${t.level})\n   ${t.desc}\n   🔑 "${t.key}"`)
				.join("\n\n");
			...
			return {
				content: [
					{
						type: "text",
						text: `**IndyDevDan's 8 TAC Tactics**

${allTactics}

📊 **Compute Advantage Equation:**
\`(Compute Scaling × Autonomy) / (Time + Effort + Cost)\`

💡 Use \`compute_advantage\` tool to calculate your leverage.`,
					},
				],
				details: undefined,
			};
...
const leveragePointsSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	category: Type.Optional(Type.String({ description: "Filter by: core, extended, or all (default)" })),
});

/**
 * 12 Leverage Points - IndyDevDan's framework for agent autonomy
 */
export function createLeveragePointsTool(): AgentTool<typeof leveragePointsSchema> {
	return {
		name: "leverage_points",
		label: "leverage_points",
		description: "Get the 12 Leverage Points for maximizing agent autonomy. Core Four + 8 Extended points.",
		parameters: leveragePointsSchema,
		execute: async (_toolCallId, { category, label }) => {
			logMcpTool("leverage_points", label);

			const coreFour = [
				{ name: "Context", desc: "Information provided to your agent. More context = better decisions." },
				{ name: "Model", desc: "The AI system you're using. Different models excel at different tasks." },
				{ name: "Prompt", desc: "How you communicate requirements. Clear prompts = reliable output." },
				{ name: "Tools", desc: "Capabilities available to the agent. Right tools = effective execution." },
			];

			const extended = [
				{ name: "Standard Out", desc: "Agent output format. Structured output enables automation." },
				{ name: "Types", desc: "Type definitions guide agent behavior and catch errors early." },
				{ name: "Tests", desc: "Automated verification. Agents can run and fix their own tests." },
				{ name: "Architecture", desc: "System design. Good architecture enables agent navigation." },
				{ name: "Documentation", desc: "Project knowledge. Agents read docs to understand context." },
				{ name: "Feedback Loops", desc: "Error signals. Agents need feedback to self-correct." },
				{ name: "Monitoring", desc: "Observability. Track agent behavior for optimization." },
				{ name: "Learning", desc: "Act-Learn-Reuse. Agents should accumulate expertise." },
			];

			const cat = category?.toLowerCase() || "all";

			let output = "**The 12 Leverage Points**\n\n";

			if (cat === "all" || cat === "core") {
				output += "🎯 **Core Four:**\n";
				output += coreFour.map((p) => `• **${p.name}**: ${p.desc}`).join("\n");
				output += "\n\n";
			}

			if (cat === "all" || cat === "extended") {
				output += "📊 **Extended Eight:**\n";
				output += extended.map((p) => `• **${p.name}**: ${p.desc}`).join("\n");
				output += "\n\n";
			}

			output += "💡 **Key Insight:** Maximize not what YOU can do, but what your AGENTS can do for you.";
...
// =============================================================================
// just-bash Integration (Vercel Labs - Secure Sandboxed Bash)
// =============================================================================

/**
 * just-bash configuration for secure agent execution
 * Based on Vercel's "We removed 80% of our agent's tools" philosophy
 */
export interface JustBashConfig {
	/** Root directory to mount (read-only via OverlayFS) */
	root?: string;
...
const justBashSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	command: Type.String({ description: "Bash command to execute" }),
	root: Type.Optional(Type.String({ description: "Root directory to mount (default: workspace)" })),
});

/**
 * Creates a just-bash sandboxed execution tool
 * Secure alternative to native bash - all writes stay in memory
 *
 * Features:
 * - OverlayFS: Reads from real filesystem, writes stay in memory
 * - Sandboxed: Cannot escape root directory
 * - Full bash: grep, sed, awk, jq, curl (if network enabled)
 * - AI-native: Designed for agent exploration
 *
 * @see https://github.com/vercel-labs/just-bash
 */
export function createJustBashTool(config: JustBashConfig = {}): AgentTool<typeof justBashSchema> {
	// Lazy import to avoid breaking if just-bash not installed
	let BashClass: any = null;
	let OverlayFsClass: any = null;

	return {
		name: "just_bash",
		label: "Secure Sandboxed Bash",
		description: `Secure sandboxed bash execution (Vercel just-bash).
All writes stay in memory - cannot modify real filesystem.
Supports: grep, sed, awk, jq, cat, ls, find, curl (if network enabled).
...
Example: grep -r "TODO" src/ | wc -l`,
		parameters: justBashSchema,
		execute: async (_toolCallId, { command, root, label }) => {
			try {
				// Dynamic import to avoid breaking if just-bash not installed
				if (!BashClass) {
					try {
						const justBash = await import("just-bash");
						BashClass = justBash.Bash;
						OverlayFsClass = justBash.OverlayFs;
					} catch {
						return {
							content: [
								{
									type: "text" as const,
									text: `just-bash not installed. Install with: npm install just-bash

Falling back to native bash is available via the Bash tool.

just-bash provides:
- Sandboxed execution (writes stay in memory)
- OverlayFS (safe read access to real files)
- Full bash compatibility (grep, sed, awk, jq)
- AI SDK integration`,
								},
							],
							details: undefined,
				...
				const mountRoot = root || config.root || process.cwd();
				const overlay = new OverlayFsClass({ root: mountRoot });
				const env = new BashClass({
					fs: overlay,
				...
				return {
					...
					details: { exitCode: result.exitCode, label },
				};
			} catch (error) {
...
const vercelInsightsSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
});

/**
 * Returns Vercel's key insights from "We removed 80% of our agent's tools"
 */
export function createVercelInsightsTool(): AgentTool<typeof vercelInsightsSchema> {
	return {
		name: "vercel_agent_insights",
		label: "Vercel Agent Insights",
		description: "Get Vercel's key insights from removing 80% of agent tools",
		parameters: vercelInsightsSchema,
		execute: async (_toolCallId, { label }) => {
			const insights = `# Vercel Agent Tool Reduction Insights

## The Experiment
Stripped internal text-to-SQL agent (d0) from 11+ specialized tools down to just 2:
- **ExecuteCommand** (bash)
...
2. **Trust the Model**
   Constraints became liabilities with Claude Opus 4.5's capabilities.

3. **Context Over Tooling**
   "Investing in documentation, clear naming, and well-structured data matters more than clever tooling."

4. **50-Year-Old Tools Win**
   "Grep is 50 years old and still does exactly what we need."

5. **Start Simple**
   Begin with Model + file system + goal. Add complexity only when proven necessary.

## The New Stack
- Model: Claude Opus 4.5 via AI SDK
- Execution: Vercel Sandbox / just-bash
- Pattern: Let agent browse files like a human analyst

## Recommendation
Use just-bash for:
- Codebase exploration
- Data analysis
- Any task where Unix tools suffice

Reserve specialized tools for:
- External API integrations
- Real-time data streams
- Actions requiring side effects
		...
		execute: async (
			...
			try {
				let result:
					...
					| Awaited<ReturnType<typeof service.list>>
					| undefined;
				let output = "";
