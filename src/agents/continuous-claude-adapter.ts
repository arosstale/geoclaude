...
export class HandoffManager extends EventEmitter {
	...
	toMarkdown(handoff: Handoff): string {
		const lines: string[] = [`# Handoff: ${handoff.title}`, "", "---", `session_id: ${handoff.sessionId}`];
...
export class SessionTracer extends EventEmitter {
	...
	private extractLearnings(_trace: SessionTrace): SessionLearning {
...
export class ReasoningHistory extends EventEmitter {
	 ...
	 */
	record(commitHash: string, attempts: ReasoningEntry["attempts"], finalSolution: string): ReasoningEntry {
	...
	search(query: string): ReasoningEntry[] {
		...
		for (const entry of this.entries.values()) {
			const matchesAttempts = entry.attempts.some(
				(a) => a.description.toLowerCase().includes(queryLower) || a.reason?.toLowerCase().includes(queryLower),
			);
