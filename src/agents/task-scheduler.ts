...
function getNextCronRun(cron: CronParts, after: Date, timezone: string): Date {
...
export class TaskSchedulerSystem extends EventEmitter {
  ...
  private executionLogs: Map<string, Array<{ level: string; message: string; data?: Record<string, unknown>; timestamp: Date }>> = new Map();
  ...
  private calculateRetryDelay(task: TaskDefinition, retryCount: number): number {
    switch (task.retryStrategy) {
      ...
      case 'exponential':
        return task.retryDelayMs * Math.pow(2, retryCount);
      default:
        return task.retryDelayMs;
    }
  }

  private scheduleRetry(task: TaskDefinition, retryCount: number, delayMs: number): void {
  ...
  updateTask(taskId: string, updates: Partial<Omit<TaskDefinition, 'id' | 'createdAt' | 'createdBy'>>): TaskDefinition | null {
    ...
    if (updates.cronExpression !== undefined || updates.intervalMs !== undefined || updates.scheduledAt !== undefined) {
      let nextRun: Date | null = null;
  ...
  enableTask(taskId: string): boolean {
    const result = this.db.prepare(`UPDATE tasks SET enabled = 1, updated_at = ? WHERE id = ?`)
  ...
  disableTask(taskId: string): boolean {
    const result = this.db.prepare(`UPDATE tasks SET enabled = 0, updated_at = ? WHERE id = ?`)
      .run(new Date().toISOString(), taskId);
  ...
  runTaskNow(taskId: string): boolean {
    ...
    this.db.prepare(`UPDATE tasks SET next_run = ? WHERE id = ?`)
      .run(new Date().toISOString(), taskId);
  ...
  getExecution(executionId: string): TaskExecution | null {
    const row = this.db.prepare(`SELECT * FROM task_executions WHERE id = ?`).get(executionId) as DbExecutionRow | undefined;
    if (!row) return null;
  ...
  addTaskToGroup(groupId: string, taskId: string): boolean {
    ...
    if (!group.tasks.includes(taskId)) {
      ...
      this.db.prepare(`UPDATE task_groups SET tasks = ? WHERE id = ?`)
        .run(JSON.stringify(group.tasks), groupId);
      return true;
    }
    return false;
  }
  ...
  getNextRuns(limit: number = 10): CronNextRun[] {
    const rows = this.db.prepare(`
      ...
      LIMIT ?
    `).all(limit) as Array<{ id: string; name: string; cron_expression: string | null; next_run: string; timezone: string }>;
