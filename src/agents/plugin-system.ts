...
export type PluginStatus = "unloaded" | "loaded" | "enabled" | "disabled" | "error" | "incompatible";
...
export class PluginSystem extends EventEmitter {
	...
	async unloadPlugin(pluginId: string): Promise<boolean> {
		...
		// Update database
		this.db.prepare(`UPDATE plugins SET status = 'unloaded', updated_at = ? WHERE id = ?`).run(Date.now(), pluginId);
	...
	async disablePlugin(pluginId: string): Promise<boolean> {
		...
		if (enabledDependents.length > 0) {
			throw new Error(`Cannot disable: required by ${enabledDependents.map((p) => p.manifest.id).join(", ")}`);
		}
	...
	private async executeHookWithTimeout(hook: PluginHook, context: HookContext): Promise<HookResult> {
		...
		return Promise.race([
			...
			new Promise<HookResult>((_, reject) => setTimeout(() => reject(new Error("Hook execution timeout")), timeout)),
		]);
	}
	...
	private startHotReloadWatcher(): void {
		...
		this.fileWatcher = fs.watch(pluginDir, { recursive: true }, (_eventType, filename) => {
			if (!filename) return;
...
export function getPluginSystem(dataDir: string, config?: Partial<PluginSystemConfig>): PluginSystem {
	if (!instance) {
