...
export class ClaudeMemClient extends EventEmitter {
    ...
    observe(content: string, options: Partial<Observation> = {}): Observation {
        ...
        if (this.currentSession) {
            ...
            this.currentSession.tokenCount = (this.currentSession.tokenCount || 0) + this.compressor.estimateTokens(sanitizedContent);
        }
    ...
    getSessionStartContext(projectPath: string): string {
        ...
        for (const session of recentSessions) {
            const compressed = this.compressor.compress(session.content, Math.floor(this.contextConfig.maxTokens / recentSessions.length));
