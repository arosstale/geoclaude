...
export class RateLimiterSystem extends EventEmitter {
	...
	private tryAcquire(
		...
		if (window.totalTokens + tokens > window.maxTokens) {
			...
			const retryAfterMs = oldestEntry ? Math.max(0, oldestEntry.timestamp + rule.windowMs - Date.now()) : 1000;
	...
	private getOrCreateBucket(scope: RateLimitScope, scopeId: string, rule: RateLimitRule): TokenBucket {
	...
	private getOrCreateWindow(scope: RateLimitScope, scopeId: string, rule: RateLimitRule): SlidingWindow {
		const key = `${scope}:${scopeId}`;
	...
	getQueueStatus(
		...
		return {
			...
			positions: queue.map((item, _index) => ({
	...
	getStats(): RateLimitStats {
		...
		const waitTimes = recentRequests.filter((r) => r.waitTimeMs !== undefined).map((r) => r.waitTimeMs!);
		...
		for (const [key, bucket] of this.buckets.entries()) {
			...
			bucketUtilization[key] = totalAvailable > 0 ? Math.round((1 - currentAvailable / totalAvailable) * 100) : 0;
		}
	...
	getEndpointStats(scope: RateLimitScope = "endpoint"): EndpointStats[] {
		...
		const recentRequests = this.requestHistory.filter((r) => r.requestedAt.getTime() >= cutoff && r.scope === scope);

		const byEndpoint: Map<
			string,
			{
				totalRequests: number;
				allowedRequests: number;
		...
		return Array.from(byEndpoint.entries()).map(([endpoint, stats]) => ({
			...
			avgLatencyMs: stats.totalRequests > 0 ? Math.round(stats.totalLatency / stats.totalRequests) : 0,
