...
export class AgentDB extends EventEmitter {
     ...
     */
    async store(key: string, content: string, namespace = "default", metadata?: Record<string, unknown>): Promise<Memory> {
...
export class HiveMind extends EventEmitter {
    ...
    async initialize(queenTask?: string): Promise<HiveAgent> {
        ...
        await this.agentDb.store("hive-init", JSON.stringify({ queenId: this.queen.id, config: this.config }), "hive-mind");
    ...
    async executeTask(task: HiveTask): Promise<HiveResult> {
        ...
        try {
            ...
            await this.agentDb.store(`task-${task.id}`, JSON.stringify({ task, result, worker: worker.id }), "hive-tasks", { success: true });
