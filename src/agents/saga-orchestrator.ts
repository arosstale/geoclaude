...
export interface SagaHistoryEntry {
  ...
  sagaId: string;
  stepId?: string;
  action: 'saga_started' | 'step_started' | 'step_completed' | 'step_failed' |
          'step_retry' | 'compensation_started' | 'compensation_completed' |
          'compensation_failed' | 'saga_completed' | 'saga_failed' | 'saga_aborted';
  timestamp: Date;
...
export class SagaOrchestrator extends EventEmitter {
  ...
  registerSaga<TContext = Record<string, unknown>>(
    ...
    stmt.run(
      ...
      JSON.stringify(definition.steps.map(s => ({
        id: s.id,
        name: s.name,
  ...
  private async executeSteps<TContext>(
    ...
    for (const level of stepLevels) {
      ...
      if (parallelSteps.length > 0) {
        const parallelPromises = parallelSteps.map(stepDef =>
          this.executeStep(saga, stepDef, definition)
        );
    ...
    for (let attempt = 0; attempt <= retryPolicy.maxRetries; attempt++) {
      try {
        ...
        const result = await this.executeWithTimeout(
          () => stepDef.execute(saga.context, saga.id),
          stepTimeoutMs
        );
  ...
  getStats(): SagaStats {
    ...
    const failureRate = totalSagas > 0
      ? (byStatus.failed + byStatus.aborted) / totalSagas
      : 0;

    const compensationRate = totalSagas > 0
      ? (byStatus.compensated + byStatus.aborted) / totalSagas
      : 0;
