...
export class AgentDiscoverySystem extends EventEmitter {
  ...
  private updateHealthScore(agentId: string): void {
    ...
    this.db
      .prepare(`UPDATE agents SET health_score = ? WHERE id = ?`)
      .run(healthScore, agentId);
  ...
  getAgent(agentId: string): RegisteredAgent | null {
    const row = this.db
      ...
      .get(agentId) as DbAgentRow | undefined;
  ...
  getAllCapabilities(): AgentCapability[] {
    const rows = this.db
      .prepare(`SELECT capabilities FROM agents WHERE status != 'offline'`)
  ...
  acquireAgent(agentId: string): boolean {
    ...
    this.db
      .prepare(`UPDATE agents SET current_load = current_load + 1 WHERE id = ?`)
      .run(agentId);
  ...
  getStats(): DiscoveryStats {
    ...
    const capabilityCounts = this.db
      .prepare(`SELECT capabilities FROM agents`)
      .all() as Array<{ capabilities: string }>;
...
export function getDiscoverySystem(
  dataDir: string,
