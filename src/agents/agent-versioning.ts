...
export class AgentVersioningSystem extends EventEmitter {
	...
	createVersion(params: {
		...
		const { agentId, config, createdBy, changelog = "", changeType = "patch", tag = "dev" } = params;
		...
		const newVersionNumber = currentVersion ? this.incrementVersion(currentVersion.version, changeType) : "1.0.0";
	...
	getVersion(versionId: string): AgentVersion | null {
		const row = this.db.prepare(`SELECT * FROM agent_versions WHERE id = ?`).get(versionId) as
			| {
					id: string;
					agent_id: string;
					version: string;
	...
	rollback(agentId: string, targetVersionId: string, userId: string): MigrationResult {
		...
		const changes = currentVersion ? this.compareVersions(currentVersion.id, targetVersionId) : [];
	 ...
	 */
	promoteVersion(versionId: string, toTag: VersionTag, userId: string): AgentVersion {
		...
		// Update tag
		this.db.prepare(`UPDATE agent_versions SET tag = ? WHERE id = ?`).run(toTag, versionId);
	...
	deleteVersion(versionId: string): boolean {
		...
		const activeCheck = this.db.prepare(`SELECT 1 FROM active_versions WHERE version_id = ?`).get(versionId);

		if (activeCheck) {
			throw new Error("Cannot delete active version");
		}

		const result = this.db.prepare(`DELETE FROM agent_versions WHERE id = ?`).run(versionId);
	...
	getStats(): VersioningStats {
		const agentCount = this.db.prepare(`SELECT COUNT(DISTINCT agent_id) as cnt FROM agent_versions`).get() as {
			cnt: number;
		};

		const versionCount = this.db.prepare(`SELECT COUNT(*) as cnt FROM agent_versions`).get() as { cnt: number };

		const tagCounts = this.db.prepare(`SELECT tag, COUNT(*) as cnt FROM agent_versions GROUP BY tag`).all() as Array<{
			tag: VersionTag;
			cnt: number;
		}>;
...
export function getVersioningSystem(dataDir: string, config?: Partial<VersioningConfig>): AgentVersioningSystem {
	if (!instance) {
