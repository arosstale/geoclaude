...
export class TAC12Client extends EventEmitter {
     ...
     */
    async scoutBuilder(prompt: string, options?: { scoutSdk?: TAC12SDKType; builderSdk?: TAC12SDKType }): Promise<TAC12Result> {
     ...
     */
    async parallel(prompt: string, sdks: TAC12SDKType[], options?: { aggregation?: "first" | "best" | "all" }): Promise<TAC12Result> {
    ...
    async getSDKs(): Promise<Array<{ sdk: TAC12SDKType; available: boolean; description: string }>> {
        return this.request<Array<{ sdk: TAC12SDKType; available: boolean; description: string }>>("GET", TAC12_ENDPOINTS.sdks);
    }

    /**
     * Get cost tracking data
     */
    async getCosts(options?: { agentId?: string; since?: Date }): Promise<{ total: number; breakdown: Record<string, number> }> {
