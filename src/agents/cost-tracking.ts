...
export class CostTrackingSystem extends EventEmitter {
	...
	recordUsage(params: {
		...
		if (pricing) {
			...
			const cachedCost = pricing.cachedInputCostPer1k ? (cachedTokens / 1000) * pricing.cachedInputCostPer1k : 0;
	...
	getAlerts(
		params: { budgetId?: string; severity?: AlertSeverity; acknowledged?: boolean; limit?: number } = {},
	): CostAlert[] {
	...
	generateReport(params: {
		...
		for (const provider of Object.values(byProvider)) {
			provider.averageCostPerCall = provider.totalCalls > 0 ? provider.totalCost / provider.totalCalls : 0;
		}
		for (const model of Object.values(byModel)) {
			model.averageCostPerCall = model.totalCalls > 0 ? model.totalCost / model.totalCalls : 0;
		}
	...
	private calculateCostTrend(records: UsageRecord[], start: Date, end: Date): CostTrendPoint[] {
		...
		for (const record of records) {
			...
			const bucketKey = groupByHour ? Math.floor(time / hourMs) * hourMs : Math.floor(time / dayMs) * dayMs;
	...
	analyzeOptimizations(): CostOptimization[] {
		...
		for (const [_modelKey, summary] of Object.entries(report.byModel)) {
		...
		const paidUsage = Object.values(report.byProvider).filter((p) =>
			["openrouter", "openai", "anthropic"].includes(p.provider),
		);
