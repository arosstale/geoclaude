...
export class CausalReasoningEngine extends EventEmitter {
    ...
    private validatePredictions(actualState: WorldState): void {
        ...
        for (const [id, prediction] of this.predictions) {
            ...
            if (similarity >= 0.8) {
                ...
                this.stateGraph.recordCounterExample(prediction.currentStateId, prediction.predictedStateId, prediction.actionId);
     ...
     */
    generateHypothesis(description: string, ifCondition: Map<string, unknown>, thenCondition: Map<string, unknown>, givenAction?: string): CausalHypothesis {
     ...
     * Register an action
     */
    registerAction(name: string, preconditions: Map<string, unknown>, expectedEffects: Map<string, unknown>): CausalAction {
    ...
    recommendActions(goalObservations: Map<string, unknown>): CausalAction[] {
        ...
        for (const action of this.actions.values()) {
            ...
            for (const [key, value] of goalObservations) {
                if (action.expectedEffects.has(key) && JSON.stringify(action.expectedEffects.get(key)) === JSON.stringify(value)) {
                    matchScore++;
                }
            }
    ...
    private updateActionStats(actionName: string, beforeState: WorldState | null, afterState: WorldState): void {
        ...
        for (const [key, value] of action.expectedEffects) {
            if (afterState.observations.has(key) && JSON.stringify(afterState.observations.get(key)) === JSON.stringify(value)) {
                effectsAchieved++;
            }
        }
...
export class TradingCausalEngine extends CausalReasoningEngine {
    ...
    observeMarket(data: {
        ...
        if (data.sentiment !== undefined) {
            ...
            observations.set("sentimentZone", data.sentiment > 0.6 ? "bullish" : data.sentiment < 0.4 ? "bearish" : "neutral");
        }
