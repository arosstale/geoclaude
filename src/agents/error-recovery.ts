...
const ERROR_PATTERNS: Array<{
  ...
  // Network errors
  { pattern: /ECONNREFUSED|ECONNRESET|ENOTFOUND|ETIMEDOUT/i, category: "network", severity: "medium", retryable: true },
  ...
  // Authentication
  { pattern: /unauthorized|401|invalid token|expired token/i, category: "authentication", severity: "high", retryable: false },
  { pattern: /auth.*fail|invalid credentials/i, category: "authentication", severity: "high", retryable: false },

  // Authorization
  { pattern: /forbidden|403|permission denied|access denied/i, category: "authorization", severity: "high", retryable: false },

  // Validation
  { pattern: /validation|invalid|malformed|bad request|400/i, category: "validation", severity: "low", retryable: false },
...
export class ErrorRecoverySystem extends EventEmitter {
  ...
  async withRetry<T>(
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      ...
      try {
        ...
        let delayMs = retryConfig.initialDelayMs * Math.pow(retryConfig.backoffMultiplier, attempt - 1);
  ...
  getStats(): ErrorStats {
    ...
    const trippedBreakers = Array.from(this.circuitBreakers.values()).filter(
      (b) => b.state === "open"
    ).length;
  ...
  isRetryable(error: Error): boolean {
    ...
    return (
      ...
      this.config.defaultRetry.retryableErrors.includes(classification.category)
    );
  }
