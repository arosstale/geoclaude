...
export class FeatureFlagsSystem extends EventEmitter {
  ...
  private cleanup(): void {
    ...
    Array.from(this.overrides.entries()).forEach(([flagKey, flagOverrides]) => {
      const validOverrides = flagOverrides.filter(
  ...
  createFlag(params: {
    ...
    // Validate percentage
    if (params.type === 'percentage' && (params.percentage === undefined || params.percentage < 0 || params.percentage > 100)) {
  ...
  updateFlag(
    key: string,
    updates: Partial<Pick<FeatureFlag, 'name' | 'description' | 'defaultValue' | 'percentage' | 'variants' | 'dependencies' | 'tags' | 'metadata' | 'expiresAt'>>,
    updatedBy: string
  ): FeatureFlag | null {
  ...
  addTarget(
    ...
    // Check for duplicate
    const existing = flag.targets.find(
      t => t.type === params.type && t.identifier === params.identifier
    );
    if (existing) {
  ...
  evaluate(flagKey: string, context: EvaluationContext = {}): EvaluationResult {
    ...
    if (this.config.enableAuditLogging && Math.random() < this.config.evaluationSampleRate) {
      this.logAudit('flag_evaluated', flagKey, context.userId ?? 'anonymous', undefined, {
        result: result.value,
  ...
  startGradualRollout(
    ...
    const intervalId = setInterval(() => {
      ...
      currentPercentage = Math.min(
        currentPercentage + params.incrementPercentage,
    ...
    for (const flag of data.flags) {
      try {
        ...
        if (data.overrides?.[flag.key]) {
          this.overrides.set(flag.key, data.overrides[flag.key].map(o => ({
            ...o,
        ...
        result.errors.push(`Failed to import flag "${flag.key}": ${error instanceof Error ? error.message : String(error)}`);
      }
    }
