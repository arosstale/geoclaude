...
export class ActionRecognizer {
	...
	parseInstruction(instruction: string): AgentStep[] {
		...
		if (lower.includes("fill") || lower.includes("enter") || lower.includes("type") || lower.includes("input")) {
			const fillMatch = instruction.match(
				/(?:fill|enter|type|input)\s+["']?([^"']+)["']?\s+(?:in|into)\s+(?:the\s+)?([^\s]+)/i,
			);
			if (fillMatch) {
				steps.push(
					this.createStep("fill", `Fill ${fillMatch[2]} with "${fillMatch[1]}"`, fillMatch[2], fillMatch[1]),
				);
			}
		}
		...
		if (lower.includes("wait")) {
			...
			const time = timeMatch ? parseInt(timeMatch[1], 10) * (timeMatch[0].includes("ms") ? 1 : 1000) : 1000;
	...
	private createStep(type: AgentStep["type"], description: string, target?: string, value?: string): AgentStep {
		return {
...
export class DataExtractor {
	 ...
	 * Extract form data
	 */
	extractForm(html: string, _formSelector?: string): ExtractedData {
		const forms: Array<{
			action?: string;
			method?: string;
...
export class BrowserUseClient extends EventEmitter {
	...
	): Promise<{ session: SandboxSession; task: AgentTask }> {
	 ...
	 */
	async fillForm(url: string, formData: Record<string, string>, submitSelector?: string): Promise<AgentTask> {
		const steps: string[] = [
			`Navigate to ${url}`,
	...
	getStatus(): {
		...
		return {
			...
			activeSandboxes: this.sandboxManager
				.getAllSessions()
				.filter((s) => s.status === "ready" || s.status === "running").length,
			config: this.config,
		};
	}
}
...
export function createSandboxedRunner(config?: SandboxConfig) {
	return async <T>(
		taskFn: (client: BrowserUseClient) => Promise<T>,
	): Promise<{ session: SandboxSession; result: T }> => {
