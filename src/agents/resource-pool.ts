...
export class ResourcePool<T> extends EventEmitter {
  ...
  async warmUp(): Promise<void> {
    ...
    for (let i = 0; i < needed; i++) {
      createPromises.push(this.createResource().then(() => {}).catch(err => {
  ...
  async acquire(options: AcquireOptions = {}): Promise<AcquireResult<T>> {
    ...
    return new Promise<AcquireResult<T>>((resolve, reject) => {
      ...
      this.emit('acquire:queued', { requestId, queuePosition: insertIndex === -1 ? this.waitingQueue.length : insertIndex + 1 });
    });
  }
    ...
    for (const resource of resources) {
      try {
        ...
        if (isHealthy) {
          ...
          this.emit('healthcheck:failed', { resourceId: resource.id, consecutiveFailures: resource.consecutiveFailures });
  ...
  getMetrics(): PoolMetrics {
    const avgAcquireTimeMs = this.stats.acquireTimes.length > 0
      ? this.stats.acquireTimes.reduce((a, b) => a + b, 0) / this.stats.acquireTimes.length
      : 0;

    const avgHoldTimeMs = this.stats.holdTimes.length > 0
