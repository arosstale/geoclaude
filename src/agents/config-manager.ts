...
export class ConfigurationManager extends EventEmitter {
  ...
  getSchema(key: string): ConfigSchema | null {
    ...
    // Load from database
    const row = this.db
      .prepare(`SELECT * FROM schemas WHERE key = ?`)
      .get(key) as DbSchemaRow | undefined;
  ...
  getAllSchemas(): ConfigSchema[] {
    const rows = this.db
      .prepare(`SELECT * FROM schemas ORDER BY category, key`)
      .all() as DbSchemaRow[];
  ...
  set(
    ...
    const storedValue = shouldEncrypt && this.encryptionKey
  ...
  delete(key: string, profile?: string): boolean {
    ...
    const result = this.db
      .prepare(`DELETE FROM config_values WHERE key = ? AND profile = ?`)
      .run(key, targetProfile);
  ...
  getAll(profile?: string): Record<string, unknown> {
    ...
    const rows = this.db
      ...
      .all(targetProfile) as DbValueRow[];
  ...
  getProfile(name: string): ConfigProfile | null {
    const row = this.db
      ...
      .get(name) as DbProfileRow | undefined;
  ...
  rollback(changeId: string, changedBy: string): boolean {
    const row = this.db
      .prepare(`SELECT * FROM config_history WHERE id = ?`)
      .get(changeId) as DbHistoryRow | undefined;
    ...
    this.db
      .prepare(`UPDATE config_history SET rolled_back = 1 WHERE id = ?`)
      .run(changeId);
  ...
  restoreSnapshot(snapshotId: string, changedBy: string): boolean {
    const row = this.db
      ...
      .get(snapshotId) as DbSnapshotRow | undefined;
  ...
  private loadValue(key: string, profile: string): ConfigValue | null {
    const row = this.db
      .prepare(`SELECT * FROM config_values WHERE key = ? AND profile = ?`)
      .get(key, profile) as DbValueRow | undefined;
  ...
  getStats(): ConfigStats {
    const keyCount = this.db
      .prepare(`SELECT COUNT(DISTINCT key) as cnt FROM config_values`)
      .get() as { cnt: number };
    ...
    const secretsCount = this.db
      .prepare(`SELECT COUNT(*) as cnt FROM schemas WHERE sensitive = 1`)
      .get() as { cnt: number };
  ...
  import(
    ...
    if (!options.merge) {
      this.db
        .prepare(`DELETE FROM config_values WHERE profile = ?`)
        .run(targetProfile);
    }
