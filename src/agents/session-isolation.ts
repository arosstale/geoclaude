...
export type SessionStatus = "active" | "suspended" | "completed" | "expired" | "error";
...
export class SessionIsolationSystem extends EventEmitter {
	...
	private createSessionProxy(metadata: SessionMetadata): IsolatedSession {
		...
		return {
			...
			set<T = unknown>(key: string, value: T): void {
				...
				if (serialized.length > self.config.maxStateSize) {
					throw new Error(`State value exceeds max size (${serialized.length} > ${self.config.maxStateSize})`);
				}
        ...
        `,
			...
			clear(): void {
				self.db.prepare(`DELETE FROM session_state WHERE session_id = ?`).run(metadata.id);
				this.touch();
			},
        ...
        `,
			...
			touch(): void {
				...
				self.db.prepare(`UPDATE sessions SET last_activity_at = ? WHERE id = ?`).run(now, metadata.id);
			},
		};
	}
	 ...
	 */
	getUserSessions(userId: string, status?: SessionStatus): SessionMetadata[] {
	...
	restoreFromSnapshot(snapshotId: string): IsolatedSession | null {
		const row = this.db.prepare(`SELECT * FROM session_snapshots WHERE id = ?`).get(snapshotId) as
			| {
					id: string;
					session_id: string;
					name: string;
    ...
    `,
		...
		for (const [id, session] of this.activeSessions) {
			if (session.metadata.expiresAt && session.metadata.expiresAt < now) {
    ...
    `,
		...
		const snapshotCount = this.db.prepare(`SELECT COUNT(*) as cnt FROM session_snapshots`).get() as { cnt: number };
...
export function getSessionIsolation(dataDir: string, config?: Partial<SessionConfig>): SessionIsolationSystem {
	if (!instance) {
