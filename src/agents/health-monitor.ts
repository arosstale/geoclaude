...
export class HealthMonitorSystem extends EventEmitter {
			...
			if (health.consecutiveFailures >= threshold.consecutiveFailures) {
				...
				if (!existingAlert) {
					this.createAlert({
						...
						previousStatus:
							health.consecutiveFailures === threshold.consecutiveFailures ? "healthy" : health.status,
	...
	private updateDegradedMode(): void {
		...
		const unhealthyCount = Array.from(this.componentHealth.values()).filter((h) => h.status === "unhealthy").length;
		...
		if (shouldBeDegraded && !this.inDegradedMode) {
			this.enterDegradedMode(
				`${unhealthyPercent.toFixed(1)}% of components unhealthy (threshold: ${this.config.degradedModeThreshold}%)`,
			);
	...
	getHistory(
		params: { checkId?: string; startDate?: Date; endDate?: Date; limit?: number } = {},
	): HealthHistoryEntry[] {
