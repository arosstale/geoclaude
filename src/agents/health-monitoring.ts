...
export class HealthMonitoringSystem extends EventEmitter {
  ...
  private updateComponentStatus(componentId: string, healthy: boolean): void {
    const component = this.db
      ...
      .get(componentId) as DbComponentRow | undefined;

    if (!component) return;

    let newStatus: HealthStatus;
    let consecutiveFailures = healthy ? 0 : component.consecutive_failures + 1;
    ...
    this.db
      .prepare(`UPDATE components SET ${updates.join(", ")} WHERE id = ?`)
      .run(...params);
  ...
  private checkThresholds(componentId: string, metricName: string, value: number): void {
    ...
    for (const threshold of thresholds) {
      ...
      if (triggered) {
        ...
        this.db
          .prepare(`UPDATE alert_thresholds SET last_triggered_at = ? WHERE id = ?`)
  ...
  getSystemHealth(): SystemHealth {
    const components = this.db
      .prepare(`SELECT * FROM components`)
      .all() as DbComponentRow[];
