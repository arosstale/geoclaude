...
export interface DiscordHookConfig {
	...
	metrics?: boolean;
	/** Enable observability hook (PAI dashboard streaming) */
...
export function createDiscordHookIntegration(config: DiscordHookConfig): HookIntegration {
	...
	if (observability) {
		const obsHook = createObservabilityHook({
			enabled: true,
			agentName: `discord-${channelId.slice(-6)}`,
		});
		manager.register(
			createHookRegistration("observability", obsHook, {
				name: "Observability",
				description: "PAI dashboard event streaming",
			}),
		);
	}
