...
class L1Cache extends EventEmitter {
	...
	getAvgGetLatency(): number {
		return this.stats.operations > 0 ? this.stats.totalGetLatency / this.stats.operations : 0;
	}

	getAvgSetLatency(): number {
		return this.stats.operations > 0 ? this.stats.totalSetLatency / this.stats.operations : 0;
	}
...
export class CacheLayerSystem extends EventEmitter {
	...
	async getOrSet<T = unknown>(key: string, loader: () => Promise<T>, options: CacheSetOptions = {}): Promise<T> {
	...
	getStats(): CacheStats {
      ...
      `);
		...
		const compressionRatio =
			this.stats.totalUncompressedSize > 0 ? this.stats.totalCompressedSize / this.stats.totalUncompressedSize : 1;
