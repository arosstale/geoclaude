...
export type MessageStatus = "pending" | "delivered" | "acknowledged" | "failed" | "expired" | "dead_letter";
...
export type SubscriptionHandler = (message: BusMessage) => Promise<undefined | unknown>;
...
export class MessageBus extends EventEmitter {
    ...
    `,
		...
		for (const { id } of expiredMessages) {
			this.db.prepare(`UPDATE messages SET status = 'expired' WHERE id = ?`).run(id);
		}
    ...
    `,
		...
		this.db.prepare(`DELETE FROM dead_letter WHERE failed_at < ?`).run(deadLetterCutoff);
	...
	private getMatchingSubscriptions(topic: string): Subscription[] {
		...
		return matching.sort((a, b) => {
			const prioA = a.options.priority ? PRIORITY_ORDER[a.options.priority] : 0;
			const prioB = b.options.priority ? PRIORITY_ORDER[b.options.priority] : 0;
			return prioB - prioA;
		});
	}
	 ...
	 */
	async reply(originalMessage: BusMessage, payload: unknown, publisherId: string): Promise<void> {
		if (!originalMessage.replyTo) {
			throw new Error("Original message does not have replyTo address");
		}

		await this.publish(originalMessage.replyTo, payload, publisherId, {
	...
	private async deliverMessage(message: BusMessage): Promise<void> {
		...
		subscriptions = subscriptions.sort((_a, _b) => {
		...
		if (this.config.enablePersistence) {
			this.db.prepare(`UPDATE messages SET status = 'delivered' WHERE id = ?`).run(message.id);
		}
	}
	 ...
	 */
	private async handleDeliveryFailure(message: BusMessage, subscription: Subscription, error: unknown): Promise<void> {
		...
		if (message.retryCount < subscription.options.maxRetries) {
			...
			const delay = subscription.options.retryDelayMs * 2 ** message.retryCount;
			...
			const timer = setTimeout(async () => {
				...
				if (this.config.enablePersistence) {
					this.db.prepare(`UPDATE messages SET retry_count = ? WHERE id = ?`).run(message.retryCount, message.id);
				}
	 ...
	 */
	private async moveToDeadLetter(message: BusMessage, error: string): Promise<void> {
      ...
      `,
			...
			this.db.prepare(`UPDATE messages SET status = 'dead_letter' WHERE id = ?`).run(message.id);
		}
	 ...
	 */
	async replay(subscriberId: string, handler: SubscriptionHandler, options: ReplayOptions = {}): Promise<number> {
		if (!this.config.enablePersistence) {
	...
	async retryDeadLetter(deadLetterId: string, publisherId: string): Promise<BusMessage | null> {
		...
		const row = this.db.prepare(`SELECT * FROM dead_letter WHERE id = ?`).get(deadLetterId) as
			| DbDeadLetterRow
			| undefined;
		...
		// Republish
		const message = await this.publish(row.topic, JSON.parse(row.payload), publisherId, { priority: "normal" });
    ...
    `,
		...
		if (result.changes > 0) {
			...
			const message = this.db.prepare(`SELECT * FROM messages WHERE id = ?`).get(messageId) as
				| DbMessageRow
				| undefined;

			if (message) {
				const ackCount = (
					this.db.prepare(`SELECT COUNT(*) as cnt FROM message_acks WHERE message_id = ?`).get(messageId) as {
						cnt: number;
					}
				).cnt;
	...
	getTopics(): string[] {
		...
		if (this.config.enablePersistence) {
			const rows = this.db.prepare(`SELECT DISTINCT topic FROM messages WHERE status = 'pending'`).all() as Array<{
				topic: string;
			}>;
	...
	getStats(): BusStats {
		...
		// Database stats
		if (this.config.enablePersistence) {
			const totalCount = (this.db.prepare(`SELECT COUNT(*) as cnt FROM messages`).get() as { cnt: number }).cnt;
			stats.totalMessages = totalCount;

			const pendingCount = (
				this.db.prepare(`SELECT COUNT(*) as cnt FROM messages WHERE status = 'pending'`).get() as { cnt: number }
			).cnt;
			stats.pendingMessages = pendingCount;

			const deadLetterCount = (this.db.prepare(`SELECT COUNT(*) as cnt FROM dead_letter`).get() as { cnt: number })
				.cnt;
	...
	getMessage(messageId: string): BusMessage | null {
		...
		const row = this.db.prepare(`SELECT * FROM messages WHERE id = ?`).get(messageId) as DbMessageRow | undefined;
...
export function getMessageBus(dataDir: string, config?: Partial<MessageBusConfig>): MessageBus {
	if (!instance) {
